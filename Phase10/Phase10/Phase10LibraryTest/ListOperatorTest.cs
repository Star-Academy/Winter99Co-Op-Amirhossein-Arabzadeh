...
namespace Phase10LibraryTest
{
    ...
    public class ListOperatorTest
    {
        ...
        private  ElasticClient _elasticClientFactory;


        public ListOperatorTest()
        {
            var docs1 = new HashSet<Doc>
            {
                new Doc("1", "ali hasan"),
                new Doc("2", "ali"),
                new Doc("3", "ali"),
            ...
            var mockSearchResponse1 = new Mock<ISearchResponse<Doc>>();
            mockSearchResponse1.Setup(x => x.Documents).Returns(docs1);
            
            var docs2 = new HashSet<Doc>
            {
                new Doc("1", "ali hasan"),
            };
            
            var mockSearchResponse2 = new Mock<ISearchResponse<Doc>>();
            mockSearchResponse2.Setup(x => x.Documents).Returns(docs2);

            
            var mockElasticClient = new Mock<IMyElasticClient>();
            mockElasticClient
                .Setup(x => x.GetSearchItemFromDb("ali"))
                .Returns(mockSearchResponse1.Object);
            mockElasticClient
                .Setup(x => x.GetSearchItemFromDb("hasan"))
                .Returns(mockSearchResponse2.Object);
            
            _listCalculator = new Mock<IListCalculator>();
            _listOperator = new ListOperator(_listCalculator.Object, mockElasticClient.Object);
        }
